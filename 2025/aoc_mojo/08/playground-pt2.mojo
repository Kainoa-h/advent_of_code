...
struct VectorPair(Comparable, Copyable, Movable, ImplicitlyCopyable):
    var v1: Vector
    var v2: Vector
    var dist: Float64
...
fn main() raises:
    with open("real.txt", "r") as f:
        ...
        var sorted_pairs = get_sorted_distance_pairs(junc_boxes)
        var circuits = List[Set[String]]()
        var qin_dynasty_ahhh_vec_pair: Optional[VectorPair] = None
        for pair in sorted_pairs:
            ...
            if len(circuits) == 1 and len(circuits[0]) == len(junc_boxes):
              qin_dynasty_ahhh_vec_pair = pair
              break

        var v1 = qin_dynasty_ahhh_vec_pair.value().v1
        var v2 = qin_dynasty_ahhh_vec_pair.value().v2
        print("v1: ", String(v1), "v2: ", String(v2))
        print(v1.x * v2.x)
