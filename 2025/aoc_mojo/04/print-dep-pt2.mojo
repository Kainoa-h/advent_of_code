...
fn create_new_grid(read old: List[List[String]])-> List[List[String]]:
  var grid = List[List[String]]()
  for row_idx in range(len(old)):
      ref row_str = old[row_idx]
      var row = List[String]()
      for col_idx in range(len(row_str)):
        row.append("@" if old[row_idx][col_idx] == "@" else ".")
      grid.append(row^)
  return grid^
